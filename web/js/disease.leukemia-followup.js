...
function loadPatientData(id) {
	...
	$.post('LoadLeukemiaBaselineServlet', $.param(params), function(response) {
		...
		localStorage.setItem("fromFollowUp3", "pass");
		window.location.replace("leukemia-baseline.jsp");
...
function actionBind() {
	$('#LeukemiaFollowUp').submit(function() {
		...
		var $form = $(this);
		if (editState == false) {
			$.post('AddLeukemiaFollowUpServlet', $form.serialize(), function(response) {
				alert("Patient added")
			}).fail(function() {
			});
		} else {
			$.post('EditLeukemiaFollowUpServlet', $form.serialize(), function(response) {
