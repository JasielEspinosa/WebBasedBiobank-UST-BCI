...
public interface SQLCommands {
	...
	String GET_PATIENT_LIST = "SELECT *, AES_DECRYPT(UNHEX(generaldatatable.LastName), '" + key + "') as LastNameDec, AES_DECRYPT(UNHEX(generaldatatable.FirstName), '" + key + "') as FirstNameDec, AES_DECRYPT(UNHEX(generaldatatable.MiddleName), '" + key + "') as MiddleNameDec FROM patienttable inner join generaldatatable on patienttable.GeneralDataID=generaldatatable.GeneralDataID where patienttable.Active = 1 and patienttable.DiseaseID = ? and (patienttable.PatientID LIKE ? or AES_DECRYPT(UNHEX(generaldatatable.LastName), '" + key + "') LIKE ?  or AES_DECRYPT(UNHEX(generaldatatable.FirstName), '" + key + "') LIKE ? or AES_DECRYPT(UNHEX(generaldatatable.MiddleName), '" + key + "') LIKE ?)";
